...
- ⬜ **记事本 v1**：被动收录所有口述（流水层）。HistoryManager 进化 + 独立窗口（快捷键唤起）+ 搜索 / 编辑 / 导出。
- ⬜ **每日总结**：当天条目自动汇总成「日报」（日期 + 标题 + 做了什么 + 重要性 + 思考），异步轻量模型。
- ⬜ **后处理 Hook 机制**：把记事本抽象成「官方 Hook #1」，为后续 Hook 留骨架。

### P2 — 开放骨架

- ⬜ **配置包导出 / 导入**：当前设置/单档导出成 `.json`，可导入。官方放几个示范模式包（正经 / 深思 / 诡辩…）。
- ⬜ **GitHub 轻量商城**：`packs/` + `index.json`，App 内拉取清单一键导入。
...
- ⬜ **音频文件转写**：拖文件进来转文字。
- ⬜ **多语言 / 自动语种检测**：仅当语言切换频繁才值得（依赖 Whisper 云端）。
- ⬜ **风格画像增强**：已发布基础版，按使用反馈调强弱。
- ⬜ **开放第三方**：社区包贡献规范、用户脚本 / 快捷指令 Hook。

### 推荐顺序

先 **P0 收尾卡顿**（已知 bug，且诊断已就位，别一直挂着）→ 再 **P1 记事本**（你最想要、最差异化；可先做成内置功能，再回头抽象成 Hook）→ 然后 **P2 配置包**（开放地基，小而稳）→ **P3 小功能**穿插补强。
